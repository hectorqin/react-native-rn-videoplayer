...
class VideoPlayer extends React.Component {
  ...
  animatedDot = (e) => {
    ...
    if (!this.state.showOpenVip && this.props.VIPCONTS) {
      e.currentTime >= this.noVipSecond &&
        this.setState(
          { showOpenVip: true, paused: true, showConts: false },
          () => {
  ...
  componentWillMount() {
    ...
    this._panResponder = PanResponder.create({
      ...
      onPanResponderGrant: (evt, gestureState) => {
        ...
        SystemSetting.getBrightness().then((brightness) => {
          ...
          if (this.brightnessData) {
            this.brightness =
              Platform.OS === "android" ? this.brightnessData : brightness;
          } else {
      ...
      onPanResponderMove: (evt, gestureState) => {
        ...
        this.soundDataY =
          (this.startY + 30 - gestureState.moveY) / this.state.height;
        this.BrightnessY =
          (this.startY + 30 - gestureState.moveY) / this.state.height;
        ...
        if (this.recordHandeY.length === this.props.panSamplingTimes) {
          if (
            ...
            if (Math.abs(this.moveYData) > 5) {
              if (
                Math.abs(this.soundDataY) <= 1 &&
                this.startX > this.state.width / 2
              ) {
                // 改变当前系统音量
                if (this.soundDataY) {
                  ...
                  if (this.soundData >= 0 && this.soundData <= 1) {
                    this.VolumeRef &&
                      this.VolumeRef.setsoundWidth((this.soundData / 1) * 100);
                  }
                  if (
                    this.soundData < 0 &&
                    this.soundData <= 1 &&
                    this.state.soundWidth != 0
                  ) {
              ...
              if (
                ...
                if (this.BrightnessY) {
                  ...
                  if (this.brightnessData >= 1) {
                    this.BrightnessRef &&
                      this.BrightnessRef.setBrightnessWidthFun(100);
                  }
                  if (this.brightnessData >= 0 && this.brightnessData <= 1) {
                    Platform.OS === "android"
                      ? NativeModules.AppBrightness.setAppBrightness(
                          this.brightnessData
                        )
                    ...
                    this.BrightnessRef &&
                      this.BrightnessRef.setBrightnessWidthFun(
                        (this.brightnessData / 1) * 100
                      );
                  }
                  if (
                    this.brightnessData < 0 &&
                    this.brightnessData <= 1 &&
                    this.state.brightnessWidth != 0
                  ) {
                    this.BrightnessRef &&
                      this.BrightnessRef.setBrightnessWidthFun(0);
                  }
                }
              }
            }
            ...
            if (this.state.onload && Math.abs(this.moveXData) > 0) {
              ...
              this.speedtime =
                duration > 60 * 30
                  ? this.realMarginLeft
                  : (this.realMarginLeft / width) * duration; //快进的时长 单位s
              this.speedalltime = getMaxdata(
                this.nowCurrentTime + this.speedtime,
                duration
              );
      ...
      onPanResponderRelease: (evt, gestureState) => {
        ...
        //调节完音量后隐藏音量显示器
        if (this.state.showVolume) {
          this.hideVolumeTime = setTimeout(() => {
            this.setState({ showVolume: false });
          }, 800);
        }
        //调节完亮度后隐藏亮度显示器
        if (this.state.showBrightness) {
          this.hideBrightnessTime = setTimeout(() => {
  ...
  changeSpeedTip = (e) => {
    this.SpeedTipTimeRef &&
      this.SpeedTipTimeRef.refs.gotimeSpeed.setNativeProps({ style: e });
  };

  //快速隐藏控件
  fastHideConts = () => {
    this.fastHide.start(() => {
      this.setState({ showConts: false, showLockCont: false });
    });
  };
  ...
  activateAutoHide = () => {
    if (this.props.autoHide === false) {
      ...
      this.TimeHideConts = setTimeout(
        this.fastHideConts,
        this.props.autoHide || 5000
      );
    }
  };
  ...
  rePlay = (autoPlay = true, admRePlay = true) => {
    ...
    if (this.state.isEnd) {
      ...
      setTimeout(() => {
        autoPlay
          ? this.setState(
              { paused: true, isEnd: false, showConts: true, ...GSTATE },
              () => {
      ...
      if (!this.state.paused) {
        this.setState({ paused: true, ...GSTATE }, () => {
          autoPlay && this.setState({ paused: false });
        });
      }
    }
  ...
  showConts = () => {
    try {
      ...
      if (this.state.showConts || this.state.showLockCont) {
        //立即消失
        this.hide.stop();
        this.fastHideConts();
      } else {
  ...
  onLoad = (data) => {
    ...
    this.toofastHide.start(() => {
      this.setState({ showConts: false });
    });
  };
  ...
  reVideo = () => {
    !this.props.repeat &&
      this.setState(
        { showConts: true, opacity: 1, paused: true, isEnd: true },
        () => {
          // this.player.seek(0)
  ...
  btnPasuedfun = () => {
    !this.state.showOpenVip &&
      ((this.adminPaused = true), this.setState({ paused: true }));
  };
  ...
  renderControllers() {
    ...
    const preShowSmallCont = smallP
      ? this.props.showSmallCont
        ? true
        : false
      : true;
    return (
      <>
        ...
        >
          ...
          {preShowSmallCont && (
            ...
            >
              {this.props.smallCloseIcon && this.state.smallP ? (
                <SvgVideoClose height="20" width="20" />
              ) : (
  ...
  render() {
    ...
    this.url = this.props.url;
    const { smallP, allTime, showOpenVip, bottomContsBottom } = this.state;
    return (
      <>
        {this.props.statusBar ? (
          smallP && this.props.statusBar()
        ) : (
        ...
        >
...

          {
            ...
            showOpenVip && this.props.VIPCONTS && (
              <ShouldPermissionTitle
                openViptipBOTTOM={bottomContsBottom + 40}
              />
            )
          }
...

          {
            ...
            this.state.showBrightness ? (
              <Brightness
                ref={(ref) => (this.BrightnessRef = ref)}
                {...this.state}
              />
            ) : null
          }
          {this.state.showChangeList &&
            this.props.renderAllSeenList &&
            this.props.renderAllSeenList()}
        </View>
      </>
    );
...
export const NgxuSetting = function () {
  this.hideAndroidBottom = () => {
    if (Platform.OS == "android") NativeModules.HideBottomNa.hide();
  };
  this.showAndroidBottom = () => {
    if (Platform.OS == "android") NativeModules.HideBottomNa.show();
  };
